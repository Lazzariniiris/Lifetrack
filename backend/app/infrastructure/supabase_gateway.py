...
class SupabaseGateway:
    async def save_analysis(self, analysis_id: str, user_id: str, token: str, result: Analysis) -> None:
        payload = {"id": analysis_id, "user_id": user_id, "foods_json": result.model_dump(mode="json"), "calories": result.nutrition.calories, "protein_g": result.nutrition.protein_g, "carbs_g": result.nutrition.carbs_g, "fat_g": result.nutrition.fat_g, "fiber_g": result.nutrition.fiber_g, "sugars_g": result.nutrition.sugars_g, "sodium_mg": result.nutrition.sodium_mg}
